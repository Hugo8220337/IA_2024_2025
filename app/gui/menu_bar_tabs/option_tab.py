...
class OptionsTab:
    ...
    def open_options(self):
        ...
        tk.Label(options_win, text="Interval between screenshots (ms):").pack(pady=5)
        ...
        tk.Checkbutton(options_win, text="Save screenshots", variable=save_var).pack(pady=5)
        ...
        tk.Checkbutton(options_win, text="Show taken screenshots", variable=show_taken_screenshots_var).pack(pady=5)

        # Screenshot path input
        screenshot_path = self.configs.get("screenshot_path", CONFIG_SCREENSHOT_DEFAULT_PATH)
        tk.Label(options_win, text="Path to save screenshots:").pack(pady=5)
        ...
        tk.Button(options_win, text="Select Folder", command=browse_path).pack(pady=5)
        ...
        def save_config():
            try:
                ...
                tk.messagebox.showerror("Error", "Please enter a valid number for the screenshot interval.")
        tk.Button(options_win, text="Save", command=save_config).pack(pady=10)